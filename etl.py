...
def process_log_data(spark, input_data, output_data):
    ...
    song_data = 'song_data/*/*/*/*.json'
    song_df = spark.read.json(os.path.join(input_data,song_data), schema = song_schema)
...
def main():
    """
    Following roles are performed by the main function :
    a) Get or create a spark session.
    b) Read the song and log data from AWS S3.
    c) Take the data and transform them to tables which will then be written to parquet files.
    d) Load the parquet files on AWS S3.
    """
